...
2. Review `README.md` / docs if the release changes developer-facing behavior
...
- validates the workspace with `cargo test`, `cargo fmt --check`, `cargo clippy`, and `cargo build --release`
- creates a GitHub Release from the tag
- uses GitHub generated release notes

For now, releases are expected to be source releases only. Binary assets can be added
later when the project decides how it wants to ship official tooling.
...
If maintainers want crate metadata to match the repository release, update the `version`
fields in:
