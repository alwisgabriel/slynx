...
pub enum LexerError {
    UnrecognizedChar {
        //indicates a character not recognized by the lexer, pointing out its index (position)
        char: char,
        index: usize,
    },
    MalformedNumber {
        //indicates where the error occurred when the lexer attempted to process a sequence as a number,
